...
class ConfigPanel:
    ...
    def _ask(self):
        ...
        self.new_values = {key: value[0] for key, value in self.new_values.items() if not value[0] is None}
        self.errors = None
...
class Question(object):
    ...
    def __init__(self, question, user_answers):
        ...
        self.ask = question.get("ask", {'en': f"{self.name}"})
    ...
    def _prevalidate(self):
        if self.value in [None, ""] and not self.optional:
            raise YunohostValidationError(
                "app_argument_required", name=self.name
            )

        # we have an answer, do some post checks
        if self.value is not None:
            if self.choices and self.value not in self.choices:
                self._raise_invalid_answer(self)
...
class NumberQuestion(Question):
    ...
    def _prevalidate(self):
        super()._prevalidate()
        if not isinstance(self.value, int) and not (isinstance(self.value, str) and self.value.isdigit()):
            raise YunohostValidationError(
...
class FileQuestion(Question):
    ...
    def _post_parse_value(self):
        ...
        if Moulinette.interface.type== 'api':
            ...
            logger.debug(f"Save uploaded file {self.value['filename']} from API into {upload_dir}")
