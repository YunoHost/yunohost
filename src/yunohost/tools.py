...
def tools_public_key():
    key_type = 'ed25519'
    keys_folder = '/etc/yunohost/keys'
    key_path = os.path.join(keys_folder, '/id_{}_ynh'.format(key_type))
    ...
    if not pub and not private:
        ...
        os.system('ssh-keygen -q -t {0} -N "" -C "Generated by Yunohost" -f {1}'.format(key_type, key_path))
